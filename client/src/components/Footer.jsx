...
const socials = [
  ...
  { Icon: Mail, href: 'mailto:hello@WealthWise.ai', label: 'Email' },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="col-span-2 lg:col-span-2">
            <Link to="/" className="flex items-center gap-3 mb-4">
              <img 
                src="/favicon.png" 
                alt="WealthWise Logo" 
                className="w-10 h-10 rounded-xl shadow-[0_0_20px_rgba(34,197,94,0.3)]"
              />
              <span className="font-display font-bold text-2xl gradient-text">WealthWise</span>
            </Link>
            <p className={`text-sm leading-relaxed mb-6 max-w-sm ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
              Empowering your financial future with AI-driven insights, real-time tracking, and personalized coaching for a life of wealth and freedom.
            </p>
        ...
        <div className={`mt-16 pt-8 border-t flex flex-col sm:flex-row items-center justify-between gap-4 ${dark ? 'border-white/10' : 'border-slate-200'}`}>
          <p className={`text-sm flex items-center gap-1.5 ${dark ? 'text-slate-500' : 'text-slate-400'}`}>
            © 2026 WealthWise. Made with <Heart className="w-3.5 h-3.5 text-pink-500 fill-pink-500" /> for smart savers.
          </p>
          <div className="flex items-center gap-6">
            <p className={`text-xs ${dark ? 'text-slate-600' : 'text-slate-400'}`}>
              AI-powered. Data-driven. Limitless.
            </p>
          </div>
        </div>
      </div>
